...
def build_slides():
    ...
    for ti, name in enumerate(TAB_ORDER):
        ...
        for wi, row in enumerate(rows, 1):
            ...
            takeaways = row[6] if len(row) > 6 else []
            ...
            slides.append(f'''<article class="slide" data-tab="{ti}" role="group" aria-roledescription="slide" aria-label="{name} {wi} of 5">
              <div class="slide-inner">
                <div class="meta"><span class="idx">{wi:02d}</span><span class="src">{esc(src)}</span>{byline}<span class="dot">&middot;</span><time class="date">{esc(fmt_date(date))}</time></div>
                <h3 class="head"><a href="{esc(url)}" target="_blank" rel="noopener">{esc(head)}</a></h3>
                {body_html}
                <a class="read" href="{esc(url)}" target="_blank" rel="noopener">Read the full story <span class="read-arrow" aria-hidden="true">{ARROW_R}</span></a>
              </div>
            </article>''')
    return "\n            ".join(slides)

DOTS = "".join('<span class="dot-i"></span>' for _ in range(5))
...
<meta name="description" content="A daily digest of the wine world, summarized into scannable key takeaways. Market, Culture, Science, and Newsletters. Every link goes to the original source.">
